...
def validate(dataloader, lengths=None):
    encoder.eval()
    decoder.eval()
    ...
    pred_list = []
    with torch.no_grad():
        for idx, (src, _) in enumerate(dataloader):
            src = src.to(device)

            states = [encoder.init_hidden(len(src)) for _ in range(4)]
            encoder_output, encoder_state, encoder_output_length = encoder(src)
            # print(encoder_output.shape)
            decoder_state = encoder_state

            decoder_input = torch.tensor([20 * [2]], device=device).reshape(-1, 1)
...
def main(args, tr_dataloader, val_dataloader):
    ...
    with open('../data/de-en/nmt_simple_len.tgt.test.npy', 'rb') as f:
        lengths = np.load(f)
    pred = validate(val_dataloader, lengths=lengths)
...
if __name__ == "__main__":
    ...
    main(args, tr_dataloader, val_dataloader)
