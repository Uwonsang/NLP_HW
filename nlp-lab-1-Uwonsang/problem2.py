...
def word_to_int(raw_data, dictionary):
...
def main(train_file, test_file):
    ...
    train_int_x = word_to_int(train_x, total_dict)
    test_int_x = word_to_int(test_x, total_dict)

    train_int_x = train_int_x.reshape(len(train_int_x), -1)
    test_int_x = test_int_x.reshape(len(test_int_x), -1)


    train_int_x, train_y = torch.LongTensor(train_int_x).to(device), torch.LongTensor(train_y).to(device)
    test_int_x = torch.LongTensor(test_int_x).to(device)

    train_dataset = TensorDataset(train_int_x, train_y)
    test_dataset = TensorDataset(test_int_x)
    ...
    model = simple_MLP_embed(vocab_size).to(device)
